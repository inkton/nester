...
class App(Cloud):
	...
	host = os.environ['NEST_CONTACT_ID'] + '@' + os.environ['NEST_APP_TAG']+".nestapp.yt"
        ...
        if (os.environ['NEST_PLATFORM_TAG'] != 'worker'):
            if self.os_exec("/usr/bin/rsync -avzrh --timeout=60 --progress "+ host +":/var/app/app.nest /var/app") < 0:
		print('Failed to download the nest project file')
		return -1
            if self.os_exec("/usr/bin/rsync -avzrh --timeout=60 --progress "+ host +":/var/app/app.json /var/app") < 0:
		print('Failed to download the nest app.json')
		return -1
            if self.os_exec("/usr/bin/rsync -avzrh --timeout=120 --progress "+ host +":/var/app/nest /var/app") < 0:
		print('Failed to download nest twigs')
		return -1
            if self.os_exec("/usr/bin/rsync -avzrh --timeout=120 --progress "+ host +":/var/app/log /var/app") < 0:
		print('Failed to download nest twigs')
		return -1
        if self.os_exec("/usr/bin/rsync -avzrh --timeout=120 --progress "+ host +":" + os.environ['NEST_FOLDER_SOURCE'] + " /var/app/source") < 0:
		print('Failed to download nest folder source')
		return -1
